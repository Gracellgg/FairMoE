...
def compute_fairness_metrics(label_list, y_pred_list, group_list, zero_division=0):
    ...
    if sum(group_list == 0) > 0:
        ...
        sklearn_TPR_group1 = _prf_divide(tp, tp + fn, zero_division)  # recall
        sklearn_TNR_group1 = _prf_divide(tn, tn + fp, zero_division)  # specificity (-recall)
    ...
    equal_odds_abs = np.abs(
        ((sklearn_TPR_group1 - sklearn_TPR_group2) + (sklearn_FPR_group1 - sklearn_FPR_group2))).mean() / 2
    ...
    fairness_metric = {'fairness/DP': demographic_parity_distance, 'fairness/EOpp0': equal_opportunity_gap_y0,
                       ...
                       'fairness/DP_abs': demographic_parity_distance_abs,
                       'fairness/EOpp0_abs': equal_opportunity_gap_y0_abs,
